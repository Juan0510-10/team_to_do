...
export default function TodoItem({ task, onToggle, onDelete, onEdit }) {
  ...
  return (
    <li className="group relative bg-black/30 rounded-3xl shadow-md hover:shadow-2xl transition-all duration-300 p-6 border border-gold/10 hover:border-gold/20 transform hover:-translate-y-1">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex items-start gap-4 flex-1 min-w-0">
          ...
          <div className="relative flex-shrink-0 mt-1">
            <input
              ...
              className="peer h-6 w-6 rounded-lg border-2 border-gold/40 text-gold focus:ring-2 focus:ring-gold/40 focus:ring-offset-1 cursor-pointer transition-all duration-200 hover:border-gold/60"
            />
            {mockTask.completed && (
              <svg 
            ...
            {isEditing ? (
              <div className="relative">
                <input
                  ...
                  className="w-full px-3 py-2 border border-gold/20 rounded-lg shadow-sm focus:ring-2 focus:ring-gold/40 focus:border-gold/40 transition-all outline-none bg-transparent text-white"
                  autoFocus
                />
              </div>
            ) : (
        ...
        {/* Botones de acción */}
          ...
          {isEditing ? (
            <>
              <button
                className="p-2 bg-gold text-black rounded-lg transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-105"
                onClick={saveEdit}
                title="Guardar"
              >
            ...
            <>
              ...
              <button
                className="p-2 bg-red-500 hover:bg-red-600 text-white rounded-lg transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-105"
